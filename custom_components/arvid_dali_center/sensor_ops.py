...
def unit_keys(devices: dict, key: str) -> list[str]:
    ...
    Родство: тот же (channel, address) + тип из пары + тот же признак `orphan`.

    ⚠ v1.2.77: `devSn` из критерия УБРАН. Факт железа, подтверждённый пользователем 2026-08-19:
    движение `0201` и освещённость `0202` ВСЕГДА сидят на одном адресе и под одним серийником,
    без исключений. Значит координата опознаёт пару не хуже серийника — и делает это в двух
    случаях, где серийник подводит:
    - **перекрёст** (docs/DEVSN_CROSSWIRE.md): шлюз меняет серийники местами, у половинок пары
      они РАЗОЙДУТСЯ → родство не выводилось, и «Забыть» сносило половину устройства;
    - **нет серийника вовсе** (лампы/датчики DALI-1, адресный режим) → прежний код возвращал
      один ключ и молча делал то же самое.

    **orphan** в критерии остаётся и обязателен: у осиротевшего адрес ТОТ ЖЕ, что у нового
    жильца, и без этого условия «Забыть» осиротевшего снесло бы живой датчик — ровно та беда,
    из-за которой на объекте 2026-08-11 пропали работавшие устройства.
    """
    ...
    if len(types) == 1:
        return [key]
    same = (dev.get("channel"), dev.get("address"), bool(dev.get("orphan")))
    out = [key]
    for k, e in devices.items():
        if k == key or str(e.get("devType")) not in types:
            continue
        if (e.get("channel"), e.get("address"), bool(e.get("orphan"))) == same:
            out.append(k)
    return out
